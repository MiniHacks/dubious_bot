...
class Client:
    def __init__(self, host, port):
        self._host = host
        self._port = port
        self._has_connected = False # We do not support re-connection, ensures we can only connect once
        self.reset_data()
    ...
    async def connect(self, loop=None):
        if not self._host or not self._port:
            raise Exception(f'Cannot connect, invalid host "{self._host}" or port {self._port}')
        
        if self._has_connected:
            raise ConnectionError('You can only connect once per object. You were in the past, to re-connect, create a new client.')
    ...
    async def _read(self):
        ...
        try:
            while not self._writer.transport.is_closing():
                ...
                with common_capnp.RawMessage.from_bytes(total_msg_buf) as msg:
