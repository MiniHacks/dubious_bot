...
class Exchange:
    def __init__(self,
        ...
        self._e = ExecClient(host=host, port=exec_port, username=username, password=password, admin_password=admin_password, max_nr_trade_history=max_nr_trade_history)
 ...
 ''')
    ...
    def insert_order(self,
                     ...
                     *,
                     ...
                     volume: int,
                     side: str,
                     order_type: str = exchange_client.ORDER_TYPE_LIMIT) -> InsertOrderResponse:
        """
        ...
        -------
        InsertOrderResponse
            The InsertOrderResponse returned by the server, containing the success flag as well as an order id and an error
            reason should the request have failed. See the doc of that type for more info.
        """
    ...
    def amend_order(self, instrument_id: str, *, order_id: int, volume: int) -> AmendOrderResponse:
        ...
        -------
        AmendOrderResponse
            The AmendOrderResponse returned by the server, containing the success flag as well as error reason
            should the request have failed. See the doc of that type for more info.
        """
        assert self.is_connected(), "Cannot call function until connected. Call connect() first"
    ...
    def delete_order(self, instrument_id: str, *, order_id: int) -> DeleteOrderResponse:
        ...
        -------
        DeleteOrderResponse
            The DeleteOrderResponse returned by the server, containing the success flag as well as error reason
            should the request have failed. See the doc of that type for more info.
        """
        assert self.is_connected(), "Cannot call function until connected. Call connect() first"
    ...
    def get_cash(self) -> float:
        ...
        -------
        float
            Returns total cash position of the client arising from all cash exchanged on previous buy and sell trades
            in all instruments.
        """
        assert self.is_connected(), "Cannot call function until connected. Call connect() first"
