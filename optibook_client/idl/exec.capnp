...
interface ExecPortal {
	...
	interface Exec {
		insertOrder @0 (instrumentId :Text, price :Float64, volume :UInt32, side :Common.Side, orderType :OrderType) -> (orderId :UInt64, success :Bool, errorReason :Text);
		amendOrder @1 (instrumentId :Text, orderId :UInt64, volume :UInt32) -> (success :Bool, errorReason :Text);
		deleteOrder @2 (instrumentId :Text, orderId :UInt64) -> (success :Bool, errorReason :Text);
		deleteOrders @3 (instrumentId :Text) -> (success :Bool, errorReason :Text);
		updateInstrumentParameters @4 (instrumentId :Text, parameters :Text) -> (success :Bool, errorReason :Text);
	}
