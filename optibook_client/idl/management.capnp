...
interface ManagementPortal {
	...
	interface Management {
		blacklist @0 (username :Text, reason :Text, tryClosePositions :Bool) -> (success :Bool, errorReason :Text);
		whitelist @1 (username :Text) -> (success :Bool, errorReason :Text);
		createInstrument @2 (instrumentId :Text, tickSize :Float64, priceChangeLimit :Common.PriceChangeLimit, extraInfo :Text);
		expireInstrument @3 (instrumentId :Text, expirationValue :Float64) -> (success :Bool, errorReason :Text);
		pauseInstrument @4 (instrumentId :Text) -> (success :Bool, errorReason :Text);
		resumeInstrument @5 (instrumentId :Text) -> (success :Bool, errorReason :Text);
		singleSidedBooking @6 (ssb :Common.SingleSidedBooking) -> (success :Bool, errorReason :Text);
